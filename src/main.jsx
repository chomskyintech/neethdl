...
function App() {
  ...
  const [solved, setSolved] = useState(() => load('hdlforge-solved', []))
  const [drafts, setDrafts] = useState(() => load('hdlforge-drafts', {}))

  const toggleSolved = (id) => {
    const next = solved.includes(id) ? solved.filter(x => x !== id) : [...solved, id]
    setSolved(next); localStorage.setItem('hdlforge-solved', JSON.stringify(next))
  }
  const saveDraft = (id, code) => {
    const next = { ...drafts, [id]: code }; setDrafts(next); localStorage.setItem('hdlforge-drafts', JSON.stringify(next))
  }
  ...
  return <div className="app">
    <header className="nav"><div className="nav-inner">
      ...
      <nav className="desktop-nav"><button className={page === 'roadmap' ? 'active' : ''} onClick={() => go('roadmap')}>Roadmap</button><button className={page === 'problems' || page === 'problem' ? 'active' : ''} onClick={() => go('problems')}>Problems</button><button className={page === 'progress' ? 'active' : ''} onClick={() => go('progress')}>Progress</button></nav>
    ...
    <div className="layout">
      ...
      <main className="main">
        ...
        {page === 'problem' && selected && <Problem problem={selected} solved={solved.includes(selected.id)} draft={drafts[selected.id]} onBack={() => go('problems')} onToggle={() => toggleSolved(selected.id)} onSave={saveDraft}/>} 
...
function Roadmap({ solved, onOpen, go }) {
  ...
  const next = problems.find(p => !solved.includes(p.id)) || problems[0]
  return <div>
    <section className="hero"><div className="eyebrow"><span className="dot"/> HARDWARE INTERVIEW ROADMAP</div><h1>Master hardware design.<br/><span>One problem at a time.</span></h1><p>Practice RTL, SystemVerilog, verification, architecture and protocols with focused, interview-style problems.</p><div className="hero-actions"><button className="primary" onClick={() => onOpen(next)}>Continue practice <ChevronRight size={17}/></button><button className="secondary" onClick={() => go('problems')}>Browse all problems</button></div></section>
...
function Problem({ problem, solved, draft, onBack, onToggle, onSave }) {
  const initial = draft ?? problem.starterCode
  const [code, setCode] = useState(initial)
  const [tab, setTab] = useState('problem')
  const [result, setResult] = useState(null)
  const [showSolution, setShowSolution] = useState(false)
  const update = (value) => { setCode(value); onSave(problem.id, value) }
  const reset = () => { setCode(problem.starterCode); onSave(problem.id, problem.starterCode); setResult(null) }
  const runChecks = () => {
    const checks = problem.checks || []
    const missing = checks.filter(c => !c.patterns.some(pattern => new RegExp(pattern, 'i').test(code)))
    const hasStarter = /Your RTL here|Write your|Add modports|Complete the FSM|Synchronizer \+ debounce logic|Fetch, drive/i.test(code)
    const pass = missing.length === 0 && !hasStarter && code.trim().length > 20
    setResult({ pass, missing, checks })
  }
  return <div><button className="back" onClick={onBack}>← Back to problems</button><div className="problem-layout"><article className="statement">
    <div className="eyebrow">{problem.category}</div><div className="problem-title-line"><h1>{problem.title}</h1><span className={`difficulty ${problem.difficulty.toLowerCase()}`}>{problem.difficulty}</span></div>
    <div className="problem-tabs"><button className={tab==='problem'?'active':''} onClick={()=>setTab('problem')}>Problem</button><button className={tab==='approach'?'active':''} onClick={()=>setTab('approach')}>Approach</button><button className={tab==='solution'?'active':''} onClick={()=>setTab('solution')}>Solution</button></div>
    {tab==='problem' && <><p className="lead">{problem.description}</p><h2>Task</h2><p>{problem.task}</p><h2>Examples</h2><div className="examples">{problem.examples.map((e,i)=><div key={i}><span>Example {i+1}</span><pre>{e}</pre></div>)}</div><h2>Constraints</h2><ul>{problem.constraints.map(c=><li key={c}>{c}</li>)}</ul></>}
    {tab==='approach' && <div className="content-card"><h2>How to think about it</h2><p>{problem.approach || 'Start by identifying the required inputs, outputs, clock/reset semantics and the state that must be remembered. Write the simplest synthesizable implementation first, then check corner cases.'}</p><h2>Interview checkpoints</h2><ul><li>Explain the timing assumptions.</li><li>Identify reset behaviour explicitly.</li><li>Consider synthesis and simulation differences.</li></ul></div>}
    {tab==='solution' && <div className="content-card"><h2>Reference solution</h2><p>Try the problem first. Reveal the reference implementation when you are ready.</p>{showSolution ? <pre className="solution-code">{problem.solution || 'Reference solution will be added for this problem.'}</pre> : <button className="secondary" onClick={()=>setShowSolution(true)}><Sparkles size={15}/> Reveal solution</button>}</div>}
  </article><aside className="editor-card"><div className="editor-head"><span><Code2 size={15}/> SystemVerilog</span><button className="icon-btn" title="Reset code" onClick={reset}><RotateCcw size={14}/></button></div><textarea value={code} onChange={e=>update(e.target.value)} spellCheck="false" aria-label="SystemVerilog editor"/><div className="editor-foot"><button className={solved?'success':'secondary'} onClick={onToggle}>{solved?<><Check size={16}/> Solved</>:<>Mark as solved</>}</button><button className="primary" onClick={runChecks}>Run checks</button></div>{result && <div className={result.pass?'run-result pass':'run-result fail'}><strong>{result.pass?'✓ Checks passed':'× Checks failed'}</strong><span>{result.pass?'Your submission passed the current browser-side checks.':'Missing or incomplete requirements: '+result.missing.map(x=>x.name).join(', ')}</span></div>}<div className="run-note">Browser checks validate the structure of your solution. Full HDL compilation, simulation and hidden testbenches are the next execution layer.</div></aside></div></div>
}
