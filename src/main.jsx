...
import {ChevronRight,Filter,Flame,Sparkles,Search,UserCircle,LayoutDashboard,BookOpen,Clock,Trophy,Zap} from 'lucide-react'
...
function Progress({problems,solved,onOpen,points,streak,topicStats}){const pct=problems.length?Math.round(solved.length/problems.length*100):0;return <div><div className="page-head"><div className="eyebrow">YOUR JOURNEY</div><h1>Progress</h1><p>Track your preparation across the HDLForge roadmap.</p></div><div className="progress-hero"><div className="ring"><strong>{pct}%</strong><span>complete</span></div><div><h2>{solved.length} of {problems.length} problems solved</h2><div className="progress-track large"><div style={{width:`${pct}%`}}/></div><p>{points} points · {streak} day streak</p></div></div><h2 className="subhead">Topic progress</h2><div className="problem-list">{Object.entries(topicStats).map(([name,stats])=><div className="problem-row" key={name}><span className="problem-main"><span><strong>{name}</strong><small>{stats.solved} of {stats.total} solved · {stats.total?Math.round(stats.solved/stats.total*100):0}%</small></span></span></div>)}</div><h2 className="subhead" style={{marginTop:'28px'}}>Solved problems</h2><div className="problem-list">{problems.filter(p=>solved.includes(p.id)).map((p,i)=><ProblemRow key={p.id} p={p} index={i} solved onOpen={onOpen}/>)}</div></div>}
