...
const problemInlineIgnore = new Set([
  'module','endmodule','input','output','inout','wire','reg','logic','bit','integer',
  'parameter','localparam','signed','unsigned','always','always_ff','always_comb',
  'assign','begin','end','if','else','case','endcase','for','while','generate',
  'entity','architecture','port','generic','signal','process','library','use',
  'std_logic','std_logic_vector','downto','to','is','of','then','elsif'
])

function problemCodeIdentifiers(problem) {
  const source = String(problem?.starterCode || '')
  const names = source.match(/\b[A-Za-z_$][\w$]*\b/g) || []
  return new Set(names.filter(name => !problemInlineIgnore.has(name.toLowerCase())))
}

function renderProblemInlineText(text, problem) {
  const identifiers = problemCodeIdentifiers(problem)
  const parts = String(text || '').split(/(`[^`]+`)/g).filter(Boolean)
  let key = 0
  return parts.flatMap(part => {
    if (part.startsWith('`') && part.endsWith('`')) {
      return [<code className="problem-inline-code" key={key++}>{part.slice(1,-1)}</code>]
    }
    return part.split(/(\b[A-Za-z_$][\w$]*\b)/g).filter(Boolean).map(piece => {
      if (identifiers.has(piece)) {
        return <code className="problem-inline-code" key={key++}>{piece}</code>
      }
...
function problemStructuredExamples(problem) {
  if (problem.displayExamples?.length) {
    return problem.displayExamples.map(example => ({
      inputLabel: example.inputLabel || 'Input:',
      input: example.input || [],
      resultLabel: example.resultLabel || 'After rising edge:',
      result: example.result || example.after || example.output || []
    }))
  }
  ...
  return (problem.examples || []).map(example => {
    ...
    const right = text.slice(arrow.index + arrow[0].length).trim()
    return {
      inputLabel: 'Input:',
      input: left.split('\n'),
      resultLabel: 'Output:',
...
export default function ProblemIDE({ problem, solved, draft, onBack, onSave, onSolved, onToggle, onPrevious, onNext, hasPrevious, hasNext, navigationLabel }) {
  ...
  const renderProblem = () => {
    const taskBullets = problem.taskBullets?.length ? problem.taskBullets : (problem.constraints || [])
    const structuredExamples = problemStructuredExamples(problem)
    const movedConstraintsIntoTask = !problem.taskBullets?.length && Boolean(problem.constraints?.length)

    return <div className="problem-copy">
      <section className="problem-section first">
        <div className="problem-heading" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'12px',padding:'20px 0 18px',borderBottom:'1px solid var(--ide-border-soft)'}}>
          <div style={{minWidth:0}}>
            <h1 style={{margin:0,color:'#e8eef5',fontSize:'20px',lineHeight:1.25,fontWeight:750,overflowWrap:'anywhere'}}>{problem.title}</h1>
            <span className="problem-category" style={{display:'inline-block',marginTop:'7px',color:'#d4d4d8',fontSize:'9px',fontWeight:800,letterSpacing:'.1em',textTransform:'uppercase'}}>{problem.topic||problem.category}</span>
          </div>
      ...
      {problem.explanation?<section className="problem-section"><h2>What this means</h2><p>{problem.explanation}</p></section>:null}
      {problem.signals?.length?<section className="problem-section"><h2>Inputs & outputs</h2><ul>{problem.signals.map((signal,index)=><li key={index}><strong>{signal.name}</strong> — {signal.direction}. {signal.description}</li>)}</ul></section>:null}

      {structuredExamples.length ? <section className="problem-section">
        <div className="structured-examples">
          {structuredExamples.map((example,index)=><div className="structured-example" key={index}>
            <div className="structured-example-title">Example {index + 1}:</div>
            <div className="structured-example-box">
              <div className="structured-example-group">
                <div className="structured-example-label input">{example.inputLabel}</div>
                <pre>{example.input.join('\n')}</pre>
              </div>
              {example.result.length ? <div className="structured-example-group">
                <div className="structured-example-label after">{example.resultLabel}</div>
                <pre>{example.result.join('\n')}</pre>
              </div> : null}
            </div>
          </div>)}
        </div>
      ...
      {!movedConstraintsIntoTask && problem.constraints?.length ? <section className="problem-section"><h2>Constraints</h2><ul>{problem.constraints.map((item,index)=><li key={index}>{item}</li>)}</ul></section> : null}
    </div>
  }
  ...
  const centralSolutions = solutions[problem.id] || {}
  const referenceSolutions = problem.solution
    ? { ...centralSolutions, SystemVerilog: centralSolutions.SystemVerilog || problem.solution }
    : centralSolutions
