...
function renderProblemInlineText(text) {
  return String(text || '').split(/(`[^`]+`)/g).filter(Boolean).map((part,index) => {
    if (part.startsWith('`') && part.endsWith('`')) {
      return <code className="problem-inline-code" key={index}>{part.slice(1,-1)}</code>
    }
...
export default function ProblemIDE({ problem, solved, draft, onBack, onSave, onSolved, onToggle, onPrevious, onNext, hasPrevious, hasNext, navigationLabel }) {
  ...
  const renderProblem = () => <div className="problem-copy"><section className="problem-section first"><div className="problem-heading" style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:'12px',padding:'20px 0 18px',borderBottom:'1px solid var(--ide-border-soft)'}}><div style={{minWidth:0}}><h1 style={{margin:0,color:'#e8eef5',fontSize:'20px',lineHeight:1.25,fontWeight:750,overflowWrap:'anywhere'}}>{problem.title}</h1><span className="problem-category" style={{display:'inline-block',marginTop:'7px',color:'#d4d4d8',fontSize:'9px',fontWeight:800,letterSpacing:'.1em',textTransform:'uppercase'}}>{problem.topic||problem.category}</span></div><span className={`difficulty ${problem.difficulty.toLowerCase()}`}>{problem.difficulty}</span></div><h2>Task</h2><div className="problem-task-copy"><p>{renderProblemInlineText(problem.task)}</p>{problem.taskBullets?.length ? <ul className="problem-task-list">{problem.taskBullets.map((item,index)=><li key={index}>{renderProblemInlineText(item)}</li>)}</ul> : null}</div></section>{problem.explanation?<section className="problem-section"><h2>What this means</h2><p>{problem.explanation}</p></section>:null}{problem.signals?.length?<section className="problem-section"><h2>Inputs & outputs</h2><ul>{problem.signals.map((signal,index)=><li key={index}><strong>{signal.name}</strong> — {signal.direction}. {signal.description}</li>)}</ul></section>:null}<section className="problem-section"><h2>Examples</h2>{problem.displayExamples?.length ? <div className="structured-examples">{problem.displayExamples.map((example,index)=><div className="structured-example" key={index}><div className="structured-example-title">Example {index + 1}:</div><div className="structured-example-box"><div className="structured-example-group"><div className="structured-example-label input">Input:</div><pre>{example.input.join('\n')}</pre></div><div className="structured-example-group"><div className="structured-example-label after">After rising edge:</div><pre>{example.after.join('\n')}</pre></div></div></div>)}</div> : <div className="examples">{problem.examples.map((example, index) => <div key={index}><span>Example {index + 1}</span><pre>{example}</pre></div>)}</div>}</section>{problem.constraints?.length ? <section className="problem-section"><h2>Constraints</h2><ul>{problem.constraints.map((item, index) => <li key={index}>{item}</li>)}</ul></section> : null}</div>
