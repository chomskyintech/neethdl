...
test.describe('HDLForge generated SEO coverage',()=>{
 ...
 test('final sitemap covers pages generated by every SEO stage',async({request})=>{
  ...
  expect(xml).toContain('https://hdlforge.netlify.app/companies/amd-hardware-design-interview/')
 })
})
...
test('problem rows stay dark and problem names use a softer neutral color',async({page})=>{
 ...
 expect(await title.evaluate(el=>getComputedStyle(el).color)).toBe('rgb(222, 222, 225)')
})
