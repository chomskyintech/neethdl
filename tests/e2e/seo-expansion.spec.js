...
test.describe('HDLForge generated SEO coverage',()=>{
 test('generates indexable pages for coding catalog problems',async({page})=>{
  await page.goto('/problems/sv-interface/index.html')
  await expect(page).toHaveTitle(/SystemVerilog Interface.*HDLForge/i)
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href','https://hdlforge.netlify.app/problems/sv-interface/')
 ...
 test('sitemap includes generated coding problem and learning URLs',async({request})=>{
  const response=await request.get('/sitemap.xml')
  expect(response.ok()).toBeTruthy()
  const xml=await response.text()
  expect(xml).toContain('https://hdlforge.netlify.app/problems/sv-interface/')
