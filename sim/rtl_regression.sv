...
// This file intentionally keeps the DUTs self-contained so the suite can run
// in open-source CI today. The testbenches exercise the same behavioural
// contracts exposed by src/data/problems.json. The DUT sections can later be
// replaced by candidate submissions without changing the test intent.
...
module test;
  ...
  initial begin
    ...
    reset=1; enable=0; @(posedge clk); #1; check(count===0,"rtl-counter reset");
    reset=0; enable=1; repeat(3) @(posedge clk); #1; check(count===3,"rtl-counter increments");
    enable=0; @(posedge clk); #1; check(count===3,"rtl-counter holds");
    ...
    reset=1; enq=0; deq=0; din=0; @(posedge clk); #1; reset=0;
    check(empty && !full,"rtl-fifo initial flags");
    enq=1; din=8'hA1; @(posedge clk); din=8'hB2; @(posedge clk); enq=0;
    check(!empty && !full,"rtl-fifo enqueue flags");
    deq=1; @(posedge clk); #1; check(dout===8'hA1,"rtl-fifo first dequeue");
    @(posedge clk); #1; check(dout===8'hB2,"rtl-fifo second dequeue"); deq=0;
    check(empty,"rtl-fifo empty after dequeue");
    enq=1; din=8'h01; @(posedge clk); din=8'h02; @(posedge clk); din=8'h03; @(posedge clk); din=8'h04; @(posedge clk); enq=0;
    #1; check(full,"rtl-fifo full");

    // 5. shift register: din enters bit 0, shifts toward MSB
    reset=1; shift_en=0; din_bit=0; @(posedge clk); reset=0;
    shift_en=1; din_bit=1; @(posedge clk); din_bit=0; @(posedge clk); #1; check(q===8'b00000010,"rtl-shift-register direction");
    shift_en=0; @(posedge clk); #1; check(q===8'b00000010,"rtl-shift-register hold");

    // 6. rising edge detector
    reset=1; signal_in=0; @(posedge clk); reset=0; #1; check(!pulse,"rtl-edge-detector reset");
    signal_in=1; @(posedge clk); #1; check(pulse,"rtl-edge-detector rising edge");
    @(posedge clk); #1; check(!pulse,"rtl-edge-detector one-cycle pulse");
    signal_in=0; @(posedge clk); #1; check(!pulse,"rtl-edge-detector falling edge");
    signal_in=1; @(posedge clk); #1; check(pulse,"rtl-edge-detector second rising edge");
    ...
    reset=1; we=0; waddr=0; wdata=0; raddr_a=0; raddr_b=0; @(posedge clk); reset=0;
    waddr=2; wdata=8'h5A; we=1; @(posedge clk); we=0; #1;
    raddr_a=2; raddr_b=0; #1; check(rdata_a===8'h5A && rdata_b===0,"rtl-regfile write/read");
    waddr=1; wdata=8'hC3; we=1; @(posedge clk); we=0; raddr_b=1; #1; check(rdata_b===8'hC3,"rtl-regfile second write");

    // 9. LFSR: reset seed and non-zero deterministic sequence
    reset=1; lfsr_enable=0; @(posedge clk); #1; check(lfsr_state===8'h01,"rtl-lfsr seed");
    reset=0; lfsr_enable=1; @(posedge clk); #1; check(lfsr_state===8'h02,"rtl-lfsr first step");
    @(posedge clk); #1; check(lfsr_state===8'h04,"rtl-lfsr second step");
    lfsr_enable=0; @(posedge clk); #1; check(lfsr_state===8'h04,"rtl-lfsr hold when disabled");

    // 10. programmable clock divider: divide-by-N toggles after N input clocks
    reset=1; div_enable=0; divide_by=3; @(posedge clk); reset=0; div_enable=1;
    check(out_clk===0,"rtl-clock-divider reset");
    @(posedge clk); #1; check(out_clk===0,"rtl-clock-divider count1");
    @(posedge clk); #1; check(out_clk===0,"rtl-clock-divider count2");
    @(posedge clk); #1; check(out_clk===1,"rtl-clock-divider toggle");
    div_enable=0; @(posedge clk); #1; check(out_clk===0,"rtl-clock-divider disable");
